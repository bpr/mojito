...
| `Float16` / `Float32` | `half` / `float` | IEEE 754 binary16 / binary32, layout 2/2 and 4/4. Arithmetic matches the VM, which computes at f64 and rounds the result to the lane once (`Dtype::round_lane`): scalar `+ - * /` widen with `fpext`, operate at `double`, and `fptrunc` back. `Float16` vector arithmetic and reductions, its rounding dunders, and its `__fma__` widen the same way, because LLVM lowers `half` arithmetic through `float` (double-rounding a product or quotient) and has no half-precision libm entries. Comparisons and negation run at the lane type. Display and `hash` read the exact f64 view (`mjrt_fmt_f64`), so no runtime entry sees a narrow float. The `half` conversions may lower to the `__extendhfdf2`/`__truncdfhf2` builtins, which the linked runtime library provides. A JIT entry returning `Float16` is refused, since stable Rust has no `f16` return type. |
| `DType` | `i8` | Holds upstream's one-byte dtype code (`Dtype::code`: `bool` 1, `int` 3, `uint8`…`int64` 134–141, `float16` 79, `float32` 81, `float64` 82). Only `==`/`!=` (an `icmp` on the code); the `is_*` queries test the code's masks (64 float, 128 sized integer, 1 signed); display selects interned names; `hash` contributes the code as a `UInt8` lane. Layout is size 1, align 1. No runtime entry sees it. |
