...
- [ ] **1.5 A comprehension keeps the clone check**
  ...
  - Each clause also records an iteration protocol, whose recipe
    (`TemplateIteration`, `template_facts/iterations.rs`) a runtime `for`
    already uses.
  - Depends on nothing.
  - Model: Opus, Not Planned.
...
- [ ] **1.10 An augmented element store keeps the clone check where the
  ...
  - A whole element read from the same list (`self.items[i] =
    ...
    cannot derive until 3.9 closes
  ...
  - No bundled body holds `x[i] += …`, so none of this moves
    `stdlib_heavy`.
  - Depends on 3.9 for the same-list read.
  - Model: Opus, Planned.
...
- [ ] **3.5 A literal passed to a `ref` parameter stops at run time**
...
- [ ] **3.6 `hash` of a struct that overloads `__hash__` stops at run time**
...
- [ ] **3.8 A generic struct's `Tuple` field over its parameter cannot be read**
...
- [ ] **3.9 An element stored from another element of the same list is rejected
  by element type**
...
- [ ] **3.11 A list literal beside a `List` parameter and a pack is not ambiguous**
...
- [ ] **3.12 Methods cannot overload on the parameter convention alone**
...
- [ ] **3.13 A trivial value handed to a `var` parameter beside a pack is always
  ambiguous**
...
- [ ] **3.14 A trivial rvalue handed to a `var` parameter beside a read overload is
  accepted**
  ...
  - This is the non-variadic sibling of 3.13, and the two
...
- [ ] **3.15 An exact constructor overload loses to its generic sibling as
  ambiguous**
...
- [ ] **3.16 A constructor overload set drops every generic candidate before it is
  ranked**
...
- [ ] **3.17 A clone that is still checked re-ranks an overloaded call**
...
- [ ] **3.19 A user `Hasher` cannot spell `update` the way the pin requires**
...
- [ ] **3.20 `Layout` carries its alignment as a runtime field**
...
- [ ] **3.21 `repr` of a sized scalar omits or misstates its type name**
...
- [ ] **3.22 A call through a `ref` to a callable value is rejected**
...
- [ ] **3.23 Forwarding a named accessor's reference result is rejected**
...
- [ ] **3.24 A generic method moving its `var` parameter into a sibling call's
...
- [ ] **3.25 A reference-returning `def` is not read through as a `print` argument**
...
- [ ] **3.26 A binder is not inferred through a converting argument**
...
- [ ] **3.28 Constructing a struct's own type parameter fails in every instance
  clone**
...
- [ ] **3.30 A `def`'s own type pack cannot be queried in a runtime position**
...
- [ ] **3.32 Explicit type arguments on a static method of a non-parametric
  struct are rejected**
...
- [ ] **3.33 A compile-time-keyed `def` cannot be passed as a function value**
...
- [ ] **3.34 An associated alias is not constructible through a parameterized
  base**
...
- [ ] **3.35 A member-led arithmetic type argument does not parse in an alias
  body**
...
- [ ] **3.36 An arithmetic `where` operand compiles for `def`s and struct methods
  only**
...
- [ ] **3.37 A tuple binding with a `Tuple[...]` annotation loses `reverse` and
  `concat`**
...
- [ ] **3.38 A reflection method call in a runtime position is rejected**
...
- [ ] **3.39 `reflect[T]` over a function's type parameter is rejected**
...
- [ ] **3.40 `reflect[T]` of a non-struct type is rejected**
...
- [ ] **3.41 Upstream `DType` names with no Mojito dtype are rejected**
...
- [ ] **3.42 A parametric nested `def` named as a value reports its marker**
...
- [ ] **3.43 `String` always owns a heap buffer, where upstream's has three
  representations**
...
- [ ] **3.44 Shift operators bind looser than the bitwise ones**
...
- [ ] **3.46 Each unrolled `comptime for` iteration shares the enclosing scope**
...
- [ ] **3.47 A trait default body holding a `comptime if` never elaborates**
...
- [ ] **3.48 A field name under a symbolic index prints without `materialize`**
  ...
  - `materialize[names[i]]()` fails at the instance in turn ("Undefined
    ...
    only.
  - Depends on 3.46 for the `comptime n = names[i]` workaround.
  - Model: Opus, Planned.

- [ ] **3.49 `comptime assert` is not parsed**
...
- [ ] **3.50 A field's value cannot be read by reflection**
  ...
  - Depends on 3.49 for the upstream spelling of the defaults.
  - Model: Fable, Planned.

- [ ] **3.51 `Int()` has no zero-argument constructor**
...
- [ ] **3.52 A `var` of an opaque type is not required to be `Deinitable`**
...
- [ ] **3.53 A local compile-time type list is unknown in an instance's annotation**
  ...
  - Depends on 3.46 when the annotation sits in a `comptime for` body.
  - Model: Opus, Not Planned.

- [ ] **3.54 A handle chain in a call's type argument is read as a value**
...
- [ ] **3.57 A subscript store on a struct with a setter and no getter is
  accepted**
...
- [ ] **3.58 `@explicit_destroy` is accepted on a type that is `Deinitable`
  unconditionally**
...
- [ ] **3.59 Mojito-specific shortcuts to move toward Mojo's shape** *(standing,
  any order)*
...
- [ ] **3.60 Behavioral divergences from the pinned Mojo — burn to zero**
  *(standing)*
