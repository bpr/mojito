...
- [ ] **1.2 A closed `SIMD` value is not a scalar of the method grammar**

  Problem: the method grammar's scalars are `Int`, `UInt`, `Bool`, and
  `Float64`, so an operator, an augmented assignment, or a store over a
  `UInt64` local keeps the body's clone check.
  - It blocks `Dict.__hash__` (`h = ((h ^ 89869747) ^ (h << 16)) * …`,
    `combined ^= h`) beside 1.1.
  - A closed construction is admitted only as a checker builtin's argument,
    a by-value argument, or a `var` local's initializer
    (`BodyShape::simd_construction`).
  - `closed_scalar` also decides the enabled classes' return types and value
    binders, so widening it there is a separate question from widening the
    grammar's operands.
  - A condition over such a value (`if error != 0:` on an `Int32` in
    `os.mkdir`, `os.remove`, `os.rmdir`) has a truthiness recipe, but its
    operator is outside the grammar; those `def`s also raise (1.33).
  - Probe: `var c = UInt64(0); c ^= UInt64(5); hasher._update_with_simd(c)`.
  - Depends on nothing.
  - Model: Opus, Not Planned.

- [ ] **1.3 A hasher fed a new leaf type keeps the clone check**
...
- [ ] **1.5 A `for` or comprehension over `range(...)` keeps the clone check**

  Problem: the method grammar admits an iterable only when it is a place of
  the body or a sibling method call's result, so a loop or comprehension
  over `range(n)` refuses the body (`BodyShape::iterable`).
  - It blocks no collection body in `stdlib_heavy` today, which loop with
    `while` instead.
  - The call records an overload target (`range$ov$Int`) and closed call
    parameters, which no instance changes, so it is a direct call of a closed
    module function like `DIRECT_CALLS` admits, bar the overload.
  - The protocol it yields is already derivable (`TemplateIteration`).
  - Depends on nothing.
  - Model: Opus, Not Planned.

- [ ] **1.6 A `with` statement keeps the clone check**
...
- [ ] **1.7 A nested `def` with explicit captures keeps the clone check**
...
- [ ] **1.8 A leading-dot contextual member keeps the clone check**
...
- [ ] **1.9 An operator with a literal or a call-result operand keeps the
  clone check**
...
- [ ] **1.10 An augmented element store keeps the clone check where the
  ...
  - A whole element read from the same list (`self.items[i] =
    ...
    cannot derive until 3.11 closes
  ...
  - No bundled body holds `x[i] += …`, so none of this moves
    `stdlib_heavy`.
  - Depends on 3.11 for the same-list read.
  - Model: Opus, Planned.

- [ ] **1.11 A method copying its receiver into a local keeps the clone
  check**
...
- [ ] **1.12 A tuple element read of a local keeps the clone check**
...
- [ ] **1.13 A surviving trait-bound template with a local of a parameter type
...
- [ ] **1.14 Every discovery round still infers every uncovered body again**
  ...
  - Depends on 1.1 through 1.12.
  - Model: Fable, Planned.

- [ ] **1.15 Binder identity is still by spelling below the checker**
...
- [ ] **1.16 A pack forwarded into a method or into `print` fails at the
  clone**
...
- [ ] **1.17 A nested variadic construction is refused on the template
  constructor path**
...
- [ ] **1.18 A per-call method clone and a whole-struct specialization leave no
  trace**
...
- [ ] **1.19 A loan-carrying instance is never cloned whole, so its replayed
...
- [ ] **1.20 A `Bool` argument does not infer a `Scalar[dt]` lane**
...
- [ ] **1.21 A method overload binding a lane through `Scalar[dt]` ties with
  a bare parameter**
...
- [ ] **1.22 A keyed method's template body cannot construct a SIMD at its own
  lane**
...
- [ ] **1.23 An inferred `SIMD[dt, _]` width leaks into the binding's declared
  type**
...
- [ ] **1.24 A keyword fieldwise construction of a variadic struct is
  rejected**
...
- [ ] **1.25 An overloaded witness whose members share an arity keeps the
  clone check**
...
- [ ] **1.26 A binder baked into a generic struct's witness keeps the clone
  check**
...
- [ ] **1.27 An overloaded method of another struct called on a place built
...
- [ ] **1.28 A module function called with explicit parameters keeps the
  clone check**
...
- [ ] **1.29 An annotated view binding keeps the clone check**
...
- [ ] **1.30 A method called on a sibling call's view result keeps the clone
  check**
...
- [ ] **1.31 A sibling call returning an immutable view keeps the clone check**
...
- [ ] **1.32 A reference result reached through an element's field keeps the
  clone check**
...
- [ ] **1.33 A raise the method grammar does not name keeps the clone check**
...
- [ ] **1.34 A folded name beside a literal keeps the clone check**
...
- [ ] **1.35 A value-keyed `def` with no compile-time control flow keeps the
  clone check**
...
- [ ] **1.36 A module-level `def` with a `var` parameter keeps the clone
  check**
...
- [ ] **1.37 A body keeping a call-through residue is never reused as its own
  template**
...
- [ ] **1.38 A `var self` method returning `self^` keeps the clone check**
...
- [ ] **1.39 An initializer collecting `var *values` keeps the clone check**
...
- [ ] **1.40 A generic module function called from a method keeps the clone
  check**
  ...
  - The body's loop (`for e in self:`) has a recipe now; this call is what
    is left.
  - The call's selection is bound once in the template
    (`hash$ov$T$Hashable`), and the instance re-keys only the application, as
    1.28 would for an explicit `[...]` application.
  - Depends on nothing.
  - Model: Opus, Not Planned.

- [ ] **1.41 A construction whose dtype or width names a parameter has no
  recipe**
  ...
  - No enabled class reaches such a body today: a `DType` or `Int` value
    ...
    runs its methods on the erased path, so the recipe waits on 1.18.
...
- [ ] **1.42 A field read of a field or of a `var` local keeps the clone
  check**
  ...
  - `Set.__iter__`'s `self.items.data` argument is the same gap (1.28).
  - A field of a closed nominal place has the same type and path under every
    instance, so only the place's binding changes.
  - Depends on nothing.
  - Model: Opus, Not Planned.

- [ ] **1.43 A string literal passed to a method's `StringSpan` parameter
  keeps the clone check**
...
- [ ] **3.11 An element stored from another element of the same list is rejected
  by element type**
...
- [ ] **3.13 A list literal beside a `List` parameter and a pack is not ambiguous**
...
- [ ] **3.14 Methods cannot overload on the parameter convention alone**
...
- [ ] **3.15 A trivial value handed to a `var` parameter beside a pack is always
  ambiguous**
...
- [ ] **3.16 A trivial rvalue handed to a `var` parameter beside a read overload is
  accepted**
  ...
  - This is the non-variadic sibling of 3.15, and the two
...
- [ ] **3.17 An exact constructor overload loses to its generic sibling as
  ambiguous**
...
- [ ] **3.18 A constructor overload set drops every generic candidate before it is
  ranked**
...
- [ ] **3.19 A clone that is still checked re-ranks an overloaded call**
...
- [ ] **3.20 A module-level `def` converts to a `def(...) capturing[_]` runtime
  parameter**
...
- [ ] **3.21 A user `Hasher` cannot spell `update` the way the pin requires**
...
- [ ] **3.22 `Layout` carries its alignment as a runtime field**
...
- [ ] **3.24 A call through a `ref` to a callable value is rejected**
...
- [ ] **3.25 Forwarding a named accessor's reference result is rejected**
...
- [ ] **3.26 A generic method moving its `var` parameter into a sibling call's
...
- [ ] **3.27 A reference-returning `def` is not read through as a `print` argument**
...
- [ ] **3.28 A binder is not inferred through a converting argument**
...
- [ ] **3.29 A bound call's result converted at an annotated binding fails
  MIR verification**
...
- [ ] **3.30 Constructing a struct's own type parameter fails in every instance
  clone**
...
- [ ] **3.32 A `def`'s own type pack cannot be queried in a runtime position**
...
- [ ] **3.34 Explicit type arguments on a static method of a non-parametric
  struct are rejected**
...
- [ ] **3.35 A compile-time-keyed `def` cannot be passed as a function value**
...
- [ ] **3.36 An associated alias is not constructible through a parameterized
  base**
...
- [ ] **3.37 A member-led arithmetic type argument does not parse in an alias
  body**
...
- [ ] **3.38 An arithmetic `where` operand compiles for `def`s and struct methods
  only**
...
- [ ] **3.39 A tuple binding with a `Tuple[...]` annotation loses `reverse` and
  `concat`**
...
- [ ] **3.40 A reflection method call in a runtime position is rejected**
...
- [ ] **3.41 `reflect[T]` over a function's type parameter is rejected**
...
- [ ] **3.42 `reflect[T]` of a non-struct type is rejected**
...
- [ ] **3.43 Upstream `DType` names with no Mojito dtype are rejected**
...
- [ ] **3.44 A parametric nested `def` named as a value reports its marker**
...
- [ ] **3.45 `String` always owns a heap buffer, where upstream's has three
  representations**
...
- [ ] **3.46 Shift operators bind looser than the bitwise ones**
...
- [ ] **3.48 Each unrolled `comptime for` iteration shares the enclosing scope**
...
- [ ] **3.49 A trait default body holding a `comptime if` never elaborates**
...
- [ ] **3.50 A field name under a symbolic index prints without `materialize`**
  ...
  - `materialize[names[i]]()` fails at the instance in turn ("Undefined
    ...
    only.
  - Depends on 3.48 for the `comptime n = names[i]` workaround.
  - Model: Opus, Planned.

- [ ] **3.51 `comptime assert` is not parsed**
...
- [ ] **3.52 A field's value cannot be read by reflection**
  ...
  - Depends on 3.51 for the upstream spelling of the defaults.
  - Model: Fable, Planned.

- [ ] **3.53 `Int()` has no zero-argument constructor**
...
- [ ] **3.54 A `var` of an opaque type is not required to be `Deinitable`**
...
- [ ] **3.55 A local compile-time type list is unknown in an instance's annotation**
  ...
  - Depends on 3.48 when the annotation sits in a `comptime for` body.
  - Model: Opus, Not Planned.

- [ ] **3.56 A handle chain in a call's type argument is read as a value**
...
- [ ] **3.57 A `mut self` witness does not conform to a read-`self`
  requirement**
...
- [ ] **3.58 An imported alias in a generic struct's method signature is an
  unknown type**
...
- [ ] **3.59 A subscript store on a struct with a setter and no getter is
  accepted**
...
- [ ] **3.60 `@explicit_destroy` is accepted on a type that is `Deinitable`
  unconditionally**
...
- [ ] **3.61 Mojito-specific shortcuts to move toward Mojo's shape** *(standing,
  any order)*
...
- [ ] **3.62 Behavioral divergences from the pinned Mojo — burn to zero**
  *(standing)*
