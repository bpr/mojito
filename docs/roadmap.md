...
- [ ] **1.3 A method's own value binder keeps the clone check**
  ...
  - The `def` recipe for a folded value parameter (`folded_literals`) is the
    ...
    source-validated, as 1.17 says for a `def`.
  - Depends on nothing.
...
- [ ] **1.6 An overloaded witness whose members share an arity keeps the
  clone check**
...
- [ ] **1.7 A binder baked into a generic struct's witness keeps the clone
  check**
...
- [ ] **1.8 An overloaded method of another struct called on a place built
...
- [ ] **1.9 A module function called with explicit parameters keeps the
  clone check**
...
- [ ] **1.10 An annotated view binding keeps the clone check**
...
- [ ] **1.11 A method called on a sibling call's view result keeps the clone
  check**
...
- [ ] **1.12 A sibling call returning an immutable view keeps the clone check**
...
- [ ] **1.13 A reference result reached through an element's field keeps the
  clone check**
...
- [ ] **1.14 A raise the method grammar does not name keeps the clone check**
...
- [ ] **1.15 A folded name beside a literal keeps the clone check**
...
- [ ] **1.16 A `rebind`-keyed method keeps the clone check**
...
- [ ] **1.17 A value-keyed `def` with no compile-time control flow keeps the
  clone check**
...
- [ ] **1.18 A module-level `def` with a `var` parameter keeps the clone
  check**
...
- [ ] **1.19 A `var self` method returning `self^` keeps the clone check**
...
- [ ] **1.20 An initializer collecting `var *values` keeps the clone check**
...
- [ ] **1.21 A generic module function called from a method keeps the clone
  check**
  ...
  - The call's selection is bound once in the template
    ...
    1.9 would for an explicit `[...]` application.
  - Depends on nothing.
  - Model: Opus, Not Planned.

- [ ] **1.22 A construction whose dtype or width names a parameter has no
  recipe**
...
- [ ] **1.23 A field read of a field or of a `var` parameter keeps the clone
  check**
  ...
  - `Set.__iter__`'s `self.items.data` argument is the same gap (1.9).
...
- [ ] **1.24 A string literal passed to a method's `StringSpan` parameter
...
- [ ] **1.25 A nested `def` beyond closed scalars keeps the clone check**
...
- [ ] **1.26 A static method of a generic struct keeps the clone check**
...
- [ ] **1.27 A method called on a read parameter keeps the clone check**
...
- [ ] **1.28 An operator with a literal left operand keeps the clone check**
...
- [ ] **1.29 A method called on a subscripted element keeps the clone
  check**
...
- [ ] **1.30 An augmented assignment to a place through its in-place dunder
  keeps the clone check**
...
- [ ] **1.31 A tuple element read as a whole value keeps the clone check**
...
- [ ] **1.32 A surviving `def` template that builds and consumes a hasher keeps
  the clone check**
...
- [ ] **1.33 A bundled template's instance over a loan-carrying argument
...
- [ ] **1.34 A per-call method clone over a loan-carrying argument keeps the
  erased path**
...
- [ ] **1.35 A frame effect whose source is not a single binding refuses
  capture**
...
- [ ] **3.20 A clone that is still checked re-ranks an overloaded call**
  ...
  - The remaining shape is outside the derivation classes: a struct method
    that calls a module-scope overload set (1.21). It closes when that
    entry lands.
  - Model: Fable, Not Planned.
...
- [ ] **3.64 Mojito-specific shortcuts to move toward Mojo's shape** *(standing,
  any order)*
...
- [ ] **3.65 Behavioral divergences from the pinned Mojo — burn to zero**
  *(standing)*
