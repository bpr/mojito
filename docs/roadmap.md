...
- [ ] **1.1 A tuple element read of a local keeps the clone check**
...
- [ ] **1.2 A surviving trait-bound template with a local of a parameter type
...
- [ ] **1.3 Every discovery round still infers every uncovered body again**
...
- [ ] **1.4 Binder identity is still by spelling below the checker**
...
- [ ] **1.5 A pack forwarded into a method or into `print` fails at the
  clone**
...
- [ ] **1.6 A nested variadic construction is refused on the template
  constructor path**
...
- [ ] **1.7 A per-call method clone and a whole-struct specialization leave no
  trace**
...
- [ ] **1.8 A loan-carrying instance is never cloned whole, so its replayed
...
- [ ] **1.9 A `Bool` argument does not infer a `Scalar[dt]` lane**
...
- [ ] **1.10 A method overload binding a lane through `Scalar[dt]` ties with
  a bare parameter**
...
- [ ] **1.11 A keyed method's template body cannot construct a SIMD at its own
  lane**
...
- [ ] **1.12 An inferred `SIMD[dt, _]` width leaks into the binding's declared
  type**
...
- [ ] **1.13 A keyword fieldwise construction of a variadic struct is
  rejected**
...
- [ ] **1.14 An overloaded witness whose members share an arity keeps the
  clone check**
...
- [ ] **1.15 A binder baked into a generic struct's witness keeps the clone
  check**
...
- [ ] **1.16 An overloaded method of another struct called on a place built
...
- [ ] **1.17 A module function called with explicit parameters keeps the
  clone check**
...
- [ ] **1.18 An annotated view binding keeps the clone check**
...
- [ ] **1.19 A method called on a sibling call's view result keeps the clone
  check**
...
- [ ] **1.20 A sibling call returning an immutable view keeps the clone check**
...
- [ ] **1.21 A reference result reached through an element's field keeps the
  clone check**
...
- [ ] **1.22 A raise the method grammar does not name keeps the clone check**
...
- [ ] **1.23 A folded name beside a literal keeps the clone check**
...
- [ ] **1.24 A `rebind`-keyed method keeps the clone check**
  ...
  - It blocks no body in `stdlib_heavy`: `Tuple.__contains__` is a pack
    struct's, refused earlier (1.7).
  - Depends on nothing.
  - Model: Opus, Not Planned.

- [ ] **1.25 A value-keyed `def` with no compile-time control flow keeps the
  clone check**
...
- [ ] **1.26 A module-level `def` with a `var` parameter keeps the clone
  check**
...
- [ ] **1.27 A body keeping a call-through residue is never reused as its own
  template**
...
- [ ] **1.28 A `var self` method returning `self^` keeps the clone check**
...
- [ ] **1.29 An initializer collecting `var *values` keeps the clone check**
...
- [ ] **1.30 A generic module function called from a method keeps the clone
  check**
  ...
  - The call's selection is bound once in the template
    ...
    1.17 would for an explicit `[...]` application.
  - Depends on nothing.
  - Model: Opus, Not Planned.

- [ ] **1.31 A construction whose dtype or width names a parameter has no
  recipe**
  ...
  - No enabled class reaches such a body today: a `DType` or `Int` value
    ...
    runs its methods on the erased path, so the recipe waits on 1.7.
...
- [ ] **1.32 A field read of a field or of a `var` parameter keeps the clone
  check**
  ...
  - `Set.__iter__`'s `self.items.data` argument is the same gap (1.17).
...
- [ ] **1.33 A string literal passed to a method's `StringSpan` parameter
  keeps the clone check**
...
- [ ] **1.34 A nested `def` beyond closed scalars keeps the clone check**
...
- [ ] **1.35 A static method of a generic struct keeps the clone check**
...
- [ ] **1.36 A method called on a read parameter keeps the clone check**
...
- [ ] **1.37 An operator with a literal left operand keeps the clone check**
...
- [ ] **1.38 A method called on a subscripted element keeps the clone
  check**
...
- [ ] **1.39 An augmented assignment to a place through its in-place dunder
  keeps the clone check**
