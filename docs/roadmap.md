...
- [ ] **1.6 A construction whose dtype or width names a parameter has no
  recipe**
...
- [ ] **1.7 A field read of a field or of a `var` parameter keeps the clone
  check**
...
- [ ] **1.8 A string literal passed to a method's `StringSpan` parameter
...
- [ ] **1.9 A nested `def` beyond closed scalars keeps the clone check**
...
- [ ] **1.10 A static method of a generic struct keeps the clone check**
...
- [ ] **1.11 A method called on a read parameter keeps the clone check**
...
- [ ] **1.12 An operator with a literal left operand keeps the clone check**
...
- [ ] **1.13 A method called on a subscripted element keeps the clone
  check**
...
- [ ] **1.14 An augmented assignment to a place through its in-place dunder
  keeps the clone check**
...
- [ ] **1.15 A tuple element read as a whole value keeps the clone check**
...
- [ ] **1.16 A surviving `def` template that builds and consumes a hasher keeps
  the clone check**
...
- [ ] **1.17 A bundled template's instance over a loan-carrying argument
...
- [ ] **1.18 A per-call method clone over a loan-carrying argument keeps the
  erased path**
...
- [ ] **1.19 A frame effect whose source is not a single binding refuses
  capture**
...
- [ ] **1.20 Native monomorphization binds type parameters by spelling**
...
- [ ] **1.21 A `where`-clause operand names its binder by spelling**
...
- [ ] **1.22 Two overloads of one generic method share their binders**
...
- [ ] **1.23 The elaborator's binders have no owner**
...
- [ ] **1.24 An overloaded witness ranked past its conversions keeps the
  clone check**
...
- [ ] **1.25 A hasher leaf under a method's own hasher binder disagrees
...
- [ ] **1.26 `os.removedirs` keeps the clone check**
...
- [ ] **1.27 A folded name in a division, comparison, or `~` keeps the clone
  check**
...
- [ ] **1.28 A `print` statement in a runtime body keeps the clone check**
