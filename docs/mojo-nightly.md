...
| String indexing | Current Mojo's String exposes byte/codepoint access through iterator and method APIs; the exact keyword-subscript spelling is not settled stable syntax. | Mojito implements the roadmap's explicit keyword-indexed forms `s[byte=i]`/`s[codepoint=i]` over the self-hosted String (positional `s[i]` rejected as ambiguous); `codepoint=` returns the scalar value as `Int` until a `Codepoint` type lands with `grapheme=`. Keyword subscripts themselves are a general Mojito feature documented in `grammar.md`. |
