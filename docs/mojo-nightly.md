...
The audit also exposed a larger adjacent surface that is deliberately tracked
separately in the roadmap: repeated trailing `where` clauses and generic
top-level `comptime` aliases. Mojito retains one clause per declaration and
rejects generic top-level aliases until the checked alias registry and
type-resolution expansion path exist; those are not parser-sized corrections.
...
| Constraints | Parameter-list `where` clauses were removed. One or more trailing declaration `where` clauses remain, and `(condition, "message")` retains a diagnostic. Type equality now uses `==`/`!=`; `_type_is_eq` was removed. Pack operands such as `Ts.values` work with `conforms_to`. | The removed placement rejects. One trailing clause, including its diagnostic tuple, runs across functions/methods, structs, conditional conformances, and supported comptime declarations; repeated clauses and generic top-level comptime aliases are the recorded subset gap. |
