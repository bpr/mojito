...
bundled contract remains monomorphic and its concrete List provenance is carried
by a checked bridge. The Mojo-only parameterized-associated-iterator fixture
keeps that subset boundary executable.
...
binary method. `current-inplace-dunders` records that Mojo-only behavior until
the first MIR-schema-prerequisite roadmap task lands.
The pinned
...
| Borrowed iteration | `Iterable` exposes `IteratorType[iterable_mut: Bool, //, iterable_origin: Origin[mut=iterable_mut]]`, and `__iter__(ref self)` returns `Self.IteratorType[origin_of(self)]`, allowing yielded references to retain source origin and mutability. `IterableOwned` separately exposes monomorphic `IteratorOwnedType`. | Concrete List borrowed/reference iteration retains executable interior provenance, but parameterized associated member declarations/applications are not represented. The bundled proof protocol therefore retains legacy `Iter`/`OwnedIter`; completing the generic current contract is the parameterized-associated-types roadmap task. |
