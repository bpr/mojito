...
| Borrowed iteration | `Iterable` exposes `IteratorType[iterable_mut: Bool, //, iterable_origin: Origin[mut=iterable_mut]]`, and `__iter__(ref self)` returns `Self.IteratorType[origin_of(self)]`, allowing yielded references to retain source origin and mutability. `IterableOwned` separately exposes monomorphic `IteratorOwnedType`. | The bundled owned protocol now uses monomorphic `IteratorOwnedType`. Parameterized associated member *declarations/applications* are represented, but resolving an `origin_of(self)` argument concretely still awaits self-origin resolution, so the borrowed `Iterable` proof protocol retains the legacy monomorphic `Iter` member; migrating it to `IteratorType[origin_of(self)]` lands with generic borrowed reference iteration. |
