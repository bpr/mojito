...
| Origin capability cast | `Origin[mut=False].cast_from[origin]` (and `ImmutableOrigin.cast_from`) downcasts an origin's capability to read-only in reference signatures; upcasts are unsafe and spelled through `MutableOrigin.cast_from` only in unsafe code. | Accept the downcast as the pinned head spells it, `ImmOrigin(o)` (equivalently `Origin[mut=False](o)`), in `ref[...]` result signatures, pinning the yielded capability to `Immutable` independent of the origin parameter's parametric `mut=`; reject the upgrade direction, and reject the removed `Origin[mut=...].cast_from[o]` spelling with a migration diagnostic. |
