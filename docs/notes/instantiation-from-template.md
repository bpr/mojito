...
`call_result_origins`. Any other record — a construction wrapping such a
view records the view's slot under the wrapping field — names the
constructed struct's slots by declaration order and its fields by name, and
is kept verbatim (`construction_immutable_binders`). A struct application's origin arguments are kept unbound too,
...
runtime parameters (a runtime body's may also be `var`, `FunctionBody`
below), a concrete scalar result (a runtime
...
| `FunctionBody(features)` | a runtime `def` body beyond the scalar classes: a whole value of any type copied or moved between a parameter, a local, a direct call's by-value argument, and the result (`OPAQUE_MOVES`, `VALUE_ARGUMENTS`), a runtime `for` over a parameter or a local (`ITERATION`), a condition tested through `__bool__` (`TRUTHINESS`), a `raises` declaration with the `raise` forms and raising calls `MethodBody` admits (`RAISES`), a `var` parameter (`OWNED_PARAMETERS`), a method call on the `^` transfer of a named place whose callee consumes its receiver (`CONSUMING_CALLS`), and a built-in operation on an untracked pointer local (`POINTER_SLOTS`: `ptr.unsafe_free()` in `dealloc`), beside `STATEMENTS`; `template_facts.rs:FUNCTION_FEATURES` is the allowlist. The allowlist is applied only to the grammar's pass over the facts: with syntax alone, `ptr.unsafe_free()` reads as a copied consuming call until the recorded type names an untracked pointer | Each argument is `MethodBody`'s for the same feature, on a body with no receiver; a `var` parameter is bound owned from its declared convention alone and rooted at its own binding under every instance, as a method's is, and a consuming method call's callee is realized from its contract, so it is the one call whose callee is not a module-scope declaration (`assets/ok/template_def_var_parameter.mojo`). A direct call's whole-value argument bound the callee's parameter exactly with the function's parameter symbolic, and the parameter's type lives in the callee's own binder scope, so the substituted application binds it exactly too and the template's selection stands; the copy or the transfer is owed again per instance, and every instance argument is plain data (obligation 12). |
...
| `OPAQUE_MOVES` | a whole value of any type moved (`^`) or copied between a parameter, a local, a field of `self` or of a `var` local, and the result, including `self` itself copied into a local (`var result = self`), moved out of it only by a `var self` receiver (`return self^`, owing `Movable` at the instance's type) | The value is never an operand, a receiver, a condition, or an argument, so nothing dispatches on its type. A store or a result has the value's own recorded type, which stays equal under substitution, so neither check converts. What a clone check still decides from the type is owed per instance (obligations 3 and 10 to 12 below). |
...
| `SIBLING_CALLS` | a method call on `self`, a field of it, a `var` local, a parameter holding a struct or its field (`value.head()`), or a call's temporary result (`self.entries().size()`, `self.name.strip().upper()`), passing closed scalars, whose contract is a `closed_method_contract`; its result may be a view over the receiver (`_DictKeyIter(self.items())`) | Obligation 7 below. Arguments are closed scalars in both checks, so nothing about them depends on the instance. A view result's `BorrowViewResult` loan and the origins its contract resolves are the callee's declared `origin_of(self)` return over the call's own receiver, which no instance changes; a constructor the view is handed to matches its field or parameter but for those origin arguments. |
| `REFERENCE_RESULT` | a method that returns a reference: every `return` hands out a field of `self`, a pointer slot, a reference call's result, or a field read through that result (`return self.entries[i].value`), of exactly the declared referent type | The `return` keeps its value as a handle because the declaration returns a reference (`ReferenceValueUses`, written from the declaration and the statement's syntax alone) and demands neither a copy nor a move. Whether the place lies within the declared origin is judged on its path, the signature, and the receiver's constructor, none of which an instance changes; the loan-escape check reads what obligation 12 already rules out. The signature is checked per clone before the body, derived or not. Any other handle in the body refuses it. |
...
| `OPERATOR_DISPATCH` | Every operator a trait names — `==`, `!=`, `<`, `<=`, `>`, `>=`, and the arithmetic, bitwise, and shift ones — over two operands of one type that mentions a struct parameter, each a place, a call result, or another such operator, or over such an operand and a literal beside a struct built over the parameter, or a closed scalar left of one | The template proves the operator through the bound, or dispatches the dunder of the struct built over the parameter, and records nothing at the operator; `infer_infix` decides a struct operand's dunder from the operand types alone (`struct_infix_dispatch`). Obligation 15 below. Neither check records anything at a temporary operand: a place is read where it lies, and a call result or an operator's value is moved into the dunder or dropped after it (`check_consuming_as` copies only a place). What the instance's dispatch adds beneath the operand is the copy of a consumed place, the conversion of an adapted one, and `NegatedEquality`; a literal's conversion into the dunder's parameter type is the template's, selected again for the instance. A literal or closed scalar left operand dispatches the right operand's reflected dunder: the template's target and `ReflectedOperator` adjustment are its own, and the instance names the target again (`struct_reflected_dispatch`). An arithmetic operator's result is the operand's own type rather than `Bool`, so it is a temporary of that type wherever the body puts it (`BodyShape::operator_value`, beside a call result and a construction). |
...
| `RAISES` | a `raises` declaration, bare or typed, and a `raise` whose operand is a `CONSTRUCTIONS` construction, `Error("…")` of a string literal, or a bare string literal; a call of a raising callee anywhere the grammar admits the call | A `raise` records nothing of its own. `require_error` asks two things of the operand's type: whether it is a string, which a constructed struct's name and the builtin `Error` settle whatever the instance, and whether it equals the declared error type, which it does under every substitution if it does symbolically, both types being written over the same parameters. The declared error type lives in the signature, checked per clone. A bare string literal's type is `StringLiteral`, closed, so it is stringish under every instance. A raising call records `EffectFacts { raises }` at the call, and a sibling's contract carries the same type (`raising_method_contract`); both are the callee's declared error type at the call's arguments, which substitute (`substituted_facts`, `realize_method_contract`), and the judgment an instance repeats — equality with the declared error type, or with the handler's in a `with` desugar — compares two functions of the same parameters. Any other effect (`may_suspend`, `diverges`) refuses (`effect_derives`). |
...
15. **Operators.** Each admitted operator is dispatched on the substituted
    ...
    adjustment. A built-in aggregate and any other type refuse. A literal
    or closed scalar left operand, which has no forward dunder for the pair,
    dispatches the right operand's reflected dunder (`1 + self.bag`,
    `n + self.bag`): the template's
    `ReflectedOperator` adjustment stands, and the instance names the target
    again at its own types (`struct_reflected_dispatch`), an overloaded
    dunder by its key, whose result must still be the type the template
    kept. The left operand is passed as it stands and records nothing.
...
26. **Nested `def` declarations.** Each nested `def`'s signature is
    substituted and installed under the instance's own statement; each
    parameter's deletability is judged again at its type, as the
    declaration's check does. Each capture, the callable's environment, and
    each call's capture accesses name the instance's own bindings; a
    capture's type is substituted, since it may be a parameter type
    (`{imm item}` over `item: Self.T`), and an owned capture owes its
    capability again at that type, as `check_capture_capability` demands:
    `ImplicitlyCopyable` for `var item`, `Movable` for `var item^`.
...
- `stdlib_heavy.mojo` checks a per-instantiation method clone 2254 times per
  ...
  *built over* the parameter instead (`Bag[T] + Bag[T]`).
  ...
  A generic module function applied explicitly in a method
  (`unsafe_alloc[Self.T](n)`, 2026-09-26) is a `DIRECT_CALLS` call: its
  application is recorded at the template's arguments, which the instance
  substitutes before obligation 5 finds the clone the elaborator
  retargeted to. The owned `__iter__`s were blocked one statement earlier
  than their roadmap entry said: the construction
  `_ListOwnedIter[Self.T](self.data, …)` hands an untracked pointer field
  to a read parameter, where no check records a copy, so the construction
  now admits such a pointer as the plain data it is. A pointer `var` local
  is a pointer slot's base too, which moves `List._realloc`.
  `stdlib_heavy` installs 598 derivations, up from 573.
  A method called on a call's temporary result (2026-09-26) moves no
  bundled count (`stdlib_heavy` installs 600 before and after): the
  receiver's read-temporary, borrow, and destruction facts are keyed by the
  call's occurrence, which no instance changes, so the grammar only had to
  admit the receiver. A user struct calling methods on a sibling's view, on
  a sibling's owned `Self`, on a field's copied list, and on a field's
  stripped string derives for two instances
  (`template_method_temporary_receiver.mojo`).
  A reference result reached through an element's field (2026-09-26)
  moves `Dict.__getitem__`: the path below the getter's reference is the
  statement's syntax, kept as a handle as a field read through a `ref`
  local is, and the referent's type substitutes, so the grammar only had to
  admit the member in `returned_place`. A user struct returning an
  element's field, read and written through, derives for two instances
  (`template_method_element_field_result.mojo`).
  A generic module function called from a method on a value of the
  struct's parameter type (`hash(e)` in `Set.__hash__`, 2026-09-26) is a
  `DIRECT_CALLS` call too: the callee's read parameter is typed by its own
  binder, which no substitution reaches, the argument is a named place the
  call reads where it lies, and the instance substitutes only the
  application. `Set.__hash__` derives, and so does a user method's
  `hash(self.value)`, which the grammar used to refuse as a non-scalar
  field (`template_method_generic_call.mojo`). The template's overload
  choice is inherited, so a method calling `pick(self.value)` beside a
  concrete `pick(x: Int)` now binds the generic member for its `Int`
  instance, as the pin does (`template_overload_binding_method.mojo`).
...
- A construction with an `ImmOrigin` or a value compile-time argument, one
  ...
  wraps a sibling's immutable view (`ImmOrigin(origin_of(self))`). A
