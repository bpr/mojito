...
| Backend-side monomorphization | `native::mono::{specialize, SpecializedProgram, MonoError}` | Native backends. Clones an entry-rooted concrete MIR graph; canonical MIR and VM execution remain unchanged. `mono/promote.rs` owns the one shape whose instance signature differs from its template's: a callable parameter bound to a closure that captures becomes the instance's last runtime parameter, renumbering the variable slots it displaces (`mir::verify::instruction_places_mut` is the shared place inventory it walks). |
