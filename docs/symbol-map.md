...
| Drops | `analysis::elaborate_drops_program` | MIR with explicit `DropVar`/`ConsumeVar` operations, plus `DropPlace`s before stores that overwrite an initialized droppable field from `analysis/store_drops.rs` (`elaborate_store_drops`, whose `out self` receiver test is `mojito_symbol::symbol::is_initializer_symbol`) and per-field `DropPlace`s for `deinit` parameters from `analysis/field_drops.rs` (`refine_deinit_fields`); re-verified before execution. |
