...
| Generics | Variadic-generic structs (`struct S[*Ts: Bound]`) | Run | Compile-time elaboration specializes each explicit instantiation into a concrete struct (mirroring pack functions; the template is dropped): pack-typed members such as `var storage: Tuple[*Ts]` expand to the concrete element list, and `s.storage[i]` has the exact per-index element type at a compile-time-constant index. Specializations copy/move/drop as ordinary structs. Current scope: exactly one trailing type pack and no other compile-time parameters; explicit bracket arguments required (no inference); runtime-varying pack indexing rejected; pack `__init__(*args)` and dependent `__getitem__[i: Int] -> Ts[i]` sugar are pending CPU-semantic work. |
