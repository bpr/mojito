...
fn parity_exe_manifest_and_differential() {
    ...
    if !focused {
        assert!(
            differential == 689,
            "exe-differential coverage must cover the complete runnable inventory: {differential} != 689"
        );
        assert!(
            errors == 36,
            "error-differential coverage must cover every runnable runtime-error fixture: {errors} != 36"
        );
        assert!(
            excluded == 0,
