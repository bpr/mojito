...
# integer lanes compare by value, float lanes compare IEEE-wise (NaN is
# unordered: every comparison but `ne` is False), and a scalar operand splats.
# They are methods because upstream's infix `<` is `Scalar`-only and its `==`
...
def main():
    ...
    var nan = zero / zero
    # `nan.ne(f)` is left out: the pin answers False there and True for
    # `nan.ne(nan)`, which no IEEE predicate does — see the
    # `simd-nan-inequality` conformance case.
    print(nan.lt(f), nan.le(f), nan.gt(f), nan.ge(f), nan.eq(f), nan.eq(nan), nan.ne(nan))
