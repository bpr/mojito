...
# reduced with `reduce_and`/`reduce_or`. Elementwise equality is `eq`/`ne`,
# since upstream's `==`/`!=` compare whole vectors.
@fieldwise_init
struct Flags:
...
def main():
    ...
    var n = SIMD[DType.bool, 8](True, True, True, True, True, True, True, True)
    print(m, n, ~m, m & n, m | ~n, m ^ n, m.eq(n), m.ne(n))
    var f = Flags(m)
    f.mask[1] = True
    f.mask[0] = False
    ...
    var gt = v.gt(4)
    print(gt, gt.shuffle[7, 6, 5, 4, 3, 2, 1, 0]())
    print(gt.select(v, -v), gt.select(v, SIMD[DType.int16, 8](0)), gt.select(SIMD[DType.int16, 8](100), v))
    print((gt & m).reduce_or(), (gt | m).reduce_and(), n.reduce_and(), (m ^ m).reduce_or())
    var b1 = SIMD[DType.bool, 2](False, True)
    print(b1, ~b1, b1.select(SIMD[DType.float64, 2](1.5, 2.5), SIMD[DType.float64, 2](0.0)))
