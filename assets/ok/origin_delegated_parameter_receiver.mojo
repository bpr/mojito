...
# at `a`. Both compilers print 1 (pin a79fbdf59f2, 2026-09-01). A second
# pointer-carrying cursor constructed from a `ref` binding resolves the same
# way.
@fieldwise_init
...
def main():
    ...
    var b = Pair(2, 20)
    ref rb = b
    var rc = RefCursor(Pointer(to=rb))
    ref j = ref_key(rc)
    print(j)
