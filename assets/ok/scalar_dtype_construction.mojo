...
# to the native Int and Float64. The width is written out: inferring it from
# the argument count (`SIMD[DType.int, _](1, 2, 3, 4)`) is the Mojito-only
# `simd-inferred-width` conformance case.
def main():
    print(Scalar[DType.uint8](300))
    print(Scalar[DType.float64](3))
    var i: Int = Scalar[DType.int](41 + 1)
    print(i)
    var v: SIMD[DType.int, 4] = SIMD[DType.int, 4](1, 2, 3, 4)
    print(v[2])
