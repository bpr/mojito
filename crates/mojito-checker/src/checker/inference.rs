...
impl Checker {
    ...
    pub(super) fn check_lambda(&mut self, expression: &Expr) -> Result<(), TypeError> {
        ...
        let ty = self
            .declaration_types
            .borrow()
            .get(&mojito_checked::checked::AnnotationSite::FunctionType {
    ...
    pub(super) fn infer_reference_value(&self, expr: &Expr) -> Option<mojito_types::origin::RefTy> {
        if let Some(mojito_checked::checked::SemanticAdjustment::ReferenceResult { reference }) =
            self.operation_adjustments.borrow().get(&expr.source_span())
        {
        ...
        match &expr.kind {
            ...
            ExprKind::Index { object, index } => match self.index_storage_ty(object, index)? {
                ...
                referent => self
                    ...
                    .get(&expr.source_span())
                    .cloned()
                    .map(|origin| mojito_types::origin::RefTy {
                        ...
                        origin: mojito_types::origin::Origin::Place(origin),
                    }),
            },
            ExprKind::TypeApply { .. } => self
                ...
                .and_then(|origin| {
                    self.infer(expr)
                        .ok()
                        .map(|referent| mojito_types::origin::RefTy {
                            referent: Box::new(referent),
                            mutability: if self.owner_is_mutable(origin.root) {
                                mojito_types::origin::Mutability::Mutable
                            } else {
        ...
        match (from, to) {
            (Ty::Ref(actual), Ty::Ref(expected)) => {
                coerces(&actual.referent, &expected.referent)
                    && (expected.mutability != mojito_types::origin::Mutability::Mutable
                        || actual.mutability == mojito_types::origin::Mutability::Mutable)
                    && match &expected.origin {
                        mojito_types::origin::Origin::Untracked { mutable } => matches!(
                            &actual.origin,
            ...
            (
                Ty::Func {
                    environment: mojito_types::origin::CallableEnvironment::Capturing(_),
                    ..
                },
                Ty::Func {
    ...
    pub(super) fn infer_impl(&self, expr: &Expr) -> Result<Ty, TypeError> {
        ...
        match &expr.kind {
            ...
            ExprKind::Invoke {
                        ...
                        .is_some_and(|info| info.methods.contains_key("__getitem__"))
                {
                    let indices: Vec<Expr> = param_args
                        ...
                        .filter_map(|argument| match argument {
                            mojito_ast::ast::ParamArg::Value(value) => Some(value.clone()),
                            _ => None,
                        })
                        .collect();
                ...
                let (ret, _, error, _, _) = self.infer_callable_ty(
                    &expr.source_span(),
                    "<callable>",
                    callable.clone(),
                    param_args,
    ...
    pub(super) fn infer_uninit_storage_construction(
        ...
        let name = mojito_types::types::UNINIT_STORAGE_TYPE_NAME;
        ...
        self.operation_adjustments.borrow_mut().insert(
            span,
            mojito_checked::checked::SemanticAdjustment::UninitStorageMake {
...
fn contextual_root_span(expression: &Expr) -> Option<mojito_common::token::SourceSpan> {
    let mut current = expression;
    loop {
        match &current.kind {
            ExprKind::Identifier(name) if name == mojito_ast::ast::CONTEXTUAL_SENTINEL => {
