...
impl Checker {
    pub(in crate::checker) fn validate_origin_signature(
        ...
        self_origin: Option<&mojito_ast::ast::OriginSpec>,
    ) -> Result<(), TypeError> {
        ...
        let validate = |spec: &mojito_ast::ast::OriginSpec| {
            for expr in spec {
    ...
    pub(in crate::checker) fn record_interior_reference(
        ...
        if self
            ...
            .get(&site)
            .is_some_and(|origin| {
                matches!(origin.path.last(), Some(mojito_types::origin::OriginSeg::Interior(tag)) if tag == name)
            })
        {
        ...
        if let Ok(mut origin) = self.origin_place(base) {
            origin
                .path
                .push(mojito_types::origin::OriginSeg::Interior(name.to_string()));
    ...
    pub(in crate::checker) fn record_aggregate_origin_invalidation_except(
        ...
        site: SourceSpan,
        origin: mojito_types::origin::Origin,
        except: Option<mojito_types::origin::OwnerId>,
    ) {
        match origin {
            mojito_types::origin::Origin::Place(place) => {
    ...
    pub(in crate::checker) fn record_origin_invalidation_kind(
        ...
        let fact = mojito_checked::checked::InteriorInvalidation {
            base,
            except,
    ...
    pub(in crate::checker) fn record_owner_invalidation(
        ...
        path: Vec<mojito_types::origin::OriginSeg>,
    ) {
        let fact = mojito_checked::checked::InteriorInvalidation {
            base: mojito_types::origin::OriginPlace { root: owner, path },
            except: None,
    ...
    pub(super) fn lookup_scoped_aggregate_origins(
        &self,
        name: &str,
    ) -> Vec<mojito_types::origin::Origin> {
    ...
    pub(in crate::checker) fn set_aggregate_origins(
        &mut self,
        name: &str,
        origins: Vec<mojito_types::origin::Origin>,
    ) {
    ...
    pub(in crate::checker) fn set_aggregate_field_origins(
        &mut self,
        name: &str,
        fields: HashMap<String, Vec<mojito_types::origin::Origin>>,
    ) {
        let Some(scope) = self.binding_scope(name) else {
            return;
        };
    ...
    pub(in crate::checker) fn aggregate_field_origins(
        ...
        fn append_unique(
            into: &mut Vec<mojito_types::origin::Origin>,
            values: impl IntoIterator<Item = mojito_types::origin::Origin>,
        ) {
            for value in values {
                                ...
                                signature
                                    ...
                                    .then(|| {
                                        self.origin_place(argument)
                                            .ok()
                                            .map(mojito_types::origin::Origin::Place)
                                            .into_iter()
                                            .collect::<Vec<_>>()
                                    })
                            })
