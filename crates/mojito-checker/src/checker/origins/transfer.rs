...
impl Checker {
    ...
    pub(in crate::checker) fn record_call_through(
        ...
        use mojito_checked::checked::{CallThroughCallee, CallThroughEffect};
    ...
    ) -> mojito_checked::checked::CallThroughArg {
        use mojito_types::origin::Origin;
        let mut out = mojito_checked::checked::CallThroughArg::default();
    ...
    pub(in crate::checker) fn apply_call_through_effects(
        &self,
        callee: &str,
        callee_decls: &[mojito_types::types::ParamDecl],
        receiver: Option<&Expr>,
        ...
        for through in &throughs {
            ...
            if let Some(identity) = self.frame_callable_identity(&concrete) {
                let composed = mojito_checked::checked::CallThroughEffect {
                    callee: identity,
                    args: through
                        .args
                        .iter()
    ...
    pub(super) fn frame_callable_identity(
        &self,
        name: &str,
    ) -> Option<mojito_checked::checked::CallThroughCallee> {
        use mojito_checked::checked::CallThroughCallee;
        let frames = self.transfer_frames.borrow();
    ...
    pub(super) fn named_callable_effects(
        &self,
        name: &str,
    ...
    pub(super) fn compose_call_through_arg(
        &self,
        arg: &mojito_checked::checked::CallThroughArg,
        receiver: Option<&Expr>,
        args: &[Expr],
    ) -> mojito_checked::checked::CallThroughArg {
        ...
        let mut out = mojito_checked::checked::CallThroughArg {
            place: None,
            carried: Vec::new(),
            local: arg.local,
        };
    ...
    pub(in crate::checker) fn replay_transfer_effects(
        ...
        for effect in effects {
            ...
            let (dest_root_owner, dest_path) = match &effect.dest {
                SigOrigin::Bound(mojito_types::origin::Origin::Place(place)) => {
            ...
            let mut sources = match &effect.src {
                ...
                src => {
                    ...
                    {
                        let origin = mojito_types::origin::Origin::Place(place);
                ...
                let derived_dest = dest_sig
                    ...
                    .or_else(|| {
                        self.owner_in_enclosing_scope(owner).then(|| {
                            SigOrigin::Bound(mojito_types::origin::Origin::Place(
                ...
                if let Some(dest_sig) = derived_dest {
                    for origin in &sources {
                            ...
                            && src_sig != dest_sig
                        {
                            ...
                            let derived = mojito_checked::checked::TransferEffect {
                                dest: dest_sig.clone(),
                                src: src_sig,
                                src_is_place,
    ...
    pub(in crate::checker) fn record_transfer_effect(
        ...
        for origin in origins {
            ...
            let effect = mojito_checked::checked::TransferEffect {
                dest: dest.clone(),
                src,
                src_is_place,
                mutable,
