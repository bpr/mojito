...
impl Checker {
    ...
    pub(super) fn iteration_binding_plan(
        ...
        use mojito_checked::checked::{CheckedIterationBinding, IterationBindingAction};
        use mojito_types::origin::Mutability;
    ...
    pub(super) fn attach_borrowed_iteration_origin(
        ...
        if source_mode != mojito_checked::checked::IterationMode::Borrowed {
            return;
        }
    ...
    pub(super) fn resolve_borrowed_iteration_reference(
        ...
        if protocol.mode != mojito_checked::checked::IterationMode::Borrowed
            || protocol.prepare.is_empty()
        {
            return None;
        }
    ...
    pub(super) fn struct_iteration_protocol(
        ...
        let convention_matched = candidates
            ...
            .filter(|sig| match mode {
                IterationMode::Owned => {
                    sig.self_convention == Some(mojito_ast::ast::ArgConvention::Var)
                }
        ...
        let checked_next = mojito_checked::checked::CheckedIteratorCall {
            target: next_symbol,
        ...
        let yield_interior = match (&next_sig.ref_return, reference_result) {
                ...
                ) =>
            {
                match &signature.origin {
                    mojito_types::origin::SigOrigin::Projected(_, path) => path.clone(),
                    _ => Vec::new(),
                }
            }
            _ => Vec::new(),
    ...
    fn require_owned_iteration_element(
        ...
        if !matches!(mode, mojito_checked::checked::IterationMode::Owned)
    ...
    pub(super) fn instantiate_iteration_method(
        ...
        signature: &MethodSig,
    ) -> Option<(Ty, Option<mojito_types::origin::RefTy>, Option<Ty>)> {
...
fn instantiate_iterator_reference(
    ...
    let mutability = match signature.mutability {
        ...
        SigMutability::BoolParam(index) => match iterator_bool_argument(arguments, index) {
            ...
            None => Mutability::Param(mojito_types::origin::OriginParamId(index as u32)),
        },
        SigMutability::Infer => match &origin {
            mojito_types::origin::Origin::Static => Mutability::Immutable,
            mojito_types::origin::Origin::Untracked { mutable: true } => Mutability::Mutable,
            mojito_types::origin::Origin::Untracked { mutable: false } => Mutability::Immutable,
            mojito_types::origin::Origin::Param(parameter) => Mutability::Param(*parameter),
...
fn iterator_bool_argument(arguments: &[TyArg], index: usize) -> Option<bool> {
    if let Some(TyArg::Val(mojito_types::ct::CtValue::Bool(value))) = arguments.get(index) {
        return Some(*value);
    }
    let mut values = arguments.iter().filter_map(|argument| match argument {
        TyArg::Val(mojito_types::ct::CtValue::Bool(value)) => Some(*value),
