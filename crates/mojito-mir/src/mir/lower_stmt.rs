...
impl Flatten<'_> {
    ...
    pub(super) fn bind_iteration_result(
        ...
        use mojito_checked::checked::IterationBindingAction;
    ...
    pub(super) fn borrow_iteration_source(
        ...
        if let Some(first_interior) = ref_origin
            .path
            .iter()
            .position(|segment| matches!(segment, mojito_types::origin::OriginSeg::Interior(_)))
        {
        ...
        let ref_ty = Ty::Ref(mojito_types::origin::RefTy {
            referent: Box::new(value_ty),
            origin: mojito_types::origin::Origin::Place(ref_origin),
    ...
    pub(super) fn lower_subscript_reference_set(
        ...
        let index_source = mojito_checked::checked::CheckedCallArgumentSource::Positional(0);
    ...
    pub(super) fn lower_augmented_subscript(
        ...
        match &target.kind {
            ExprKind::Index { object, index } => {
                ...
                let (receiver, receiver_place) = self.lower_call_receiver(object);
                let index_source =
                    mojito_checked::checked::CheckedCallArgumentSource::Positional(0);
                let retain_index =
                ...
                let value_source = if value_keyword {
                    ...
                    mojito_checked::checked::CheckedCallArgumentSource::Positional(1)
                };
            ...
            ExprKind::MultiIndex {
                ...
                let value_source = if value_keyword {
                    ...
                    mojito_checked::checked::CheckedCallArgumentSource::Positional(source.len())
                };
                let value = self.apply_checked_call_value_adjustments(
                    setter,
                    value_source,
    ...
    pub(super) fn lower_subscript_set(&mut self, target: &Expr, value_expression: &Expr) -> bool {
        ...
        let (object, source_arguments): (&Expr, Option<&[mojito_ast::ast::SubscriptArg]>) =
            match &target.kind {
        ...
        if let Some(arguments) = source_arguments {
            for argument in arguments {
                if let mojito_ast::ast::SubscriptArg::Index(argument) = argument {
                ...
                for (i, (target, extraction)) in targets.iter().zip(plan).enumerate() {
                    ...
                    let call = extraction.accessor.clone().map(|target| MirSubscriptCall {
                        ...
                        receiver_convention: extraction
                            .reference
                            .as_ref()
                            .map(|_| mojito_ast::ast::ArgConvention::Ref),
                        arguments: Vec::new(),
                        capture_accesses: Vec::new(),
    ...
    pub(super) fn lower_hir_stmt(
        &mut self,
        statement: &mojito_hir::hir::HirStmt,
        outer_map: &HashMap<hir::BlockId, MirBlockId>,
    ) {
        let mut index = HashMap::new();
