    ...
    match instruction {
        ...
        MirInstr::UninitStorage { dest, init } => {
            ...
            if let Some(init) = init
                && let Some(element) =
                    reg_ty(dest).and_then(mojito_types::types::uninit_storage_element)
        ...
        MirInstr::CallIndirect {
            ...
            let stored_contract =
                reg_ty(callee).and_then(mojito_types::types::callable_contract_ty);
            ...
            if let Some(target) = resolved {
                if nominal_name.is_none()
                    && let Some(expected) =
                        reg_ty(callee).and_then(mojito_symbol::symbol::callable_contract_target)
                    && target != &expected
                {
                    errors.push(format!(
