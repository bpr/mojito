    ...
    pub(in crate::mir) fn expr_unconverted(&mut self, e: &Expr) -> Reg {
        match &e.kind {
            ...
            ExprKind::MethodCall {
                ...
                if let Some(mojito_checked::checked::SemanticAdjustment::FieldInvocation {
                    callable,
                }) = self.checked_adjustments(e).into_iter().find(|adjustment| {
                    matches!(
                        adjustment,
                ...
                let pointer_storage = self.checked_adjustments(e).into_iter().find_map(
                    |adjustment| match adjustment {
                        mojito_checked::checked::SemanticAdjustment::PointerStorageTake {
                            element,
                        } => Some((true, element)),
                        mojito_checked::checked::SemanticAdjustment::PointerStorageDestroy {
                            element,
                        } => Some((false, element)),
                        _ => None,
                    },
                ...
                let uninit_storage = self.checked_adjustments(e).into_iter().find_map(
                    |adjustment| match adjustment {
                        mojito_checked::checked::SemanticAdjustment::UninitStorageWrite {
                            element,
                        } => Some((UninitStorageOp::Write, element)),
                        mojito_checked::checked::SemanticAdjustment::UninitStorageTake {
                            element,
                        } => Some((UninitStorageOp::Take, element)),
                        mojito_checked::checked::SemanticAdjustment::UninitStorageDestroy {
                            element,
            ...
            ExprKind::MultiIndex { object, args } => {
                ...
                if matches!(self.checked_ty(object), Some(Ty::Pointer { .. }))
                    && let [mojito_ast::ast::SubscriptArg::Keyword { name, value }] =
                        args.as_slice()
                    && name == "unsafe_offset"
                {
                    if let Some(place) = self.pointer_deref_place(object) {
                ...
                let lowered_args: Vec<_> = args
                    ...
                    .filter_map(|(argument, descriptor)| match argument {
                        mojito_ast::ast::SubscriptArg::Keyword { name, value } => {
