...
pub fn lower_cfg(cfg: &Cfg) -> MirFunction {
    lower_cfg_nested(
        cfg,
        &HashMap::new(),
        &mojito_symbol::symbol::OverloadSets::default(),
        false,
        &[],
        &[],
        &HashMap::new(),
...
fn mir_place_handle_ty(
    place: &MirPlace,
    requested: Option<mojito_types::origin::Mutability>,
) -> Option<Ty> {
    ...
    let source_mutability = match root {
        ...
        Ty::Pointer { origin, .. } => match origin {
            mojito_types::origin::PointerOrigin::Place { mutable, .. } => Some(if *mutable {
            ...
            | mojito_types::origin::PointerOrigin::SelfPlace { mutability, .. } => {
                Some(*mutability)
            }
            mojito_types::origin::PointerOrigin::Static
            | mojito_types::origin::PointerOrigin::Untracked { .. }
            | mojito_types::origin::PointerOrigin::UnsafeAny { .. } => None,
        },
        _ => None,
    };
    ...
    let mutability = source_mutability
        .or(requested)
        .unwrap_or(mojito_types::origin::Mutability::Mutable);
    Some(Ty::Ref(mojito_types::origin::RefTy {
        referent: Box::new(storage),
        origin: mojito_types::origin::Origin::Untracked {
...
struct Flatten<'a> {
    ...
    owner_vars: HashMap<mojito_types::origin::OwnerId, VarId>,
    ...
    transfer_domain_loans: HashMap<(VarId, Vec<mojito_types::origin::OriginSeg>), Vec<MirLoan>>,
...
impl Flatten<'_> {
            ...
            .or_else(|| {
                self.checked_adjustments(expression)
                    ...
                    .find_map(|adjustment| match adjustment {
                        mojito_checked::checked::SemanticAdjustment::ResolveCallable(target) => {
                            Some(target)
                        }
                        _ => None,
                    })
    ...
    fn implicit_conversion(&self, expression: &Expr) -> Option<String> {
        self.checked_adjustments(expression)
            .into_iter()
            .find_map(|adjustment| match adjustment {
                mojito_checked::checked::SemanticAdjustment::ImplicitConversion(target) => {
                    Some(target)
                }
                _ => None,
            })
    ...
    fn index_normalization(&self, expression: &Expr) -> Option<String> {
        self.checked_adjustments(expression)
            .into_iter()
            .find_map(|adjustment| match adjustment {
                mojito_checked::checked::SemanticAdjustment::IndexNormalization { target } => {
                    Some(target)
                }
                _ => None,
            })
    ...
    fn implicitly_copies_consuming_receiver(&self, expression: &Expr) -> bool {
        self.checked_adjustments(expression)
            ...
            .any(|adjustment| {
                matches!(
                    adjustment,
                    mojito_checked::checked::SemanticAdjustment::ImplicitlyCopyConsumingReceiver
                )
            })
    }
    ...
    fn overloaded_name(&self, name: &str, argc: usize) -> String {
        if self.overloads.function_is_overloaded(name, argc) {
            mojito_symbol::symbol::unresolved_overload_marker(name, argc)
        } else {
            name.to_string()
        }
    }
    ...
    fn declare_binding_var(&mut self, binding: mojito_types::origin::OwnerId, name: &str) -> VarId {
        if let Some(var) = self.owner_vars.get(&binding).copied() {
            return var;
        }
    ...
    fn param_arg_reg(&mut self, argument: &ParamArg, site: &SourceSpan) -> Option<Reg> {
        ...
        if let Some(name) = adjustments.iter().find_map(|adjustment| match adjustment {
            mojito_checked::checked::SemanticAdjustment::ReifyTypeArgument { name } => {
                Some(name.clone())
            }
            _ => None,
        }) {
...
struct ExprFacts {
    ty: Option<Ty>,
    place_ty: Option<Ty>,
    owner: Option<mojito_types::origin::OwnerId>,
    raises: Option<Ty>,
    adjustments: Vec<mojito_checked::checked::SemanticAdjustment>,
    comprehension_bindings: Vec<mojito_checked::checked::CheckedComprehensionBinding>,
}
...
struct NestedCapture {
    ...
    binding: mojito_types::origin::OwnerId,
    ty: Ty,
    kind: mojito_ast::ast::CaptureKind,
}
    ...
    match &expression.kind {
        ...
        ExprKind::Comprehension {
            ...
            .map(|clause| match clause {
                mojito_ast::ast::ComprehensionClause::For { iter, .. } => iter.as_ref(),
                mojito_ast::ast::ComprehensionClause::If(condition) => condition.as_ref(),
            })
        ...
        ExprKind::MultiIndex { object, args } => {
            ...
            for argument in args {
                match argument {
                    ...
                    | mojito_ast::ast::SubscriptArg::Keyword { value, .. } => children.push(value),
...
struct TryRegions<'a> {
    ...
    handler_binding: Option<mojito_types::origin::OwnerId>,
}
