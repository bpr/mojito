...
impl Flatten<'_> {
    ...
    pub(in crate::mir) fn expr_hir(&mut self, expression: &crate::hir::HirExpr) -> Reg {
    ...
    pub(in crate::mir) fn reference_handle_hir(&mut self, expression: &crate::hir::HirExpr) -> Reg {
        let mut index = HashMap::new();
        index_hir_expression(&expression.syntax, expression, &mut index);
        self.active_semantics.push(index);
    ...
    pub(in crate::mir) fn place_hir(&mut self, expression: &crate::hir::HirExpr) -> MirPlace {
        let mut index = HashMap::new();
        index_hir_expression(&expression.syntax, expression, &mut index);
        self.active_semantics.push(index);
