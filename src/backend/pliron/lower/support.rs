...
pub(in crate::backend::pliron) fn emit_thunk_body(
    ctx: &mut Context,
    func: FuncOp,
    target: &FnSignature,
    modes: &str,
...
pub(in crate::backend::pliron) fn emit_pow_body(ctx: &mut Context, func: FuncOp) {
    let entry = func.get_or_create_entry_block(ctx);
    let region = func
        .get_operation()
        .deref(ctx)
...
pub(in crate::backend::pliron) fn c_abi_type(
    ctx: &mut Context,
    ty: crate::native::rt_abi::CAbiTy,
) -> TypeHandle {
...
pub(in crate::backend::pliron) fn collect_projected_move_places<'m>(
...
pub(in crate::backend::pliron) fn collect_aliased_receiver_regs(
...
pub(in crate::backend::pliron) fn collect_loaded_places(
    blocks: &[MirBlock],
) -> HashMap<u32, MirPlace> {
...
pub(in crate::backend::pliron) fn scalar_copy_ty(ty: &Ty) -> Option<ScalarTy> {
    match ty {
...
pub(in crate::backend::pliron) fn is_aggregate_ty(ty: &Ty) -> bool {
    matches!(
        ty,
...
pub(in crate::backend::pliron) fn scalar_type(
    function: &str,
    ty: &Ty,
...
pub(in crate::backend::pliron) fn is_comparison(op: InfixOp) -> bool {
    matches!(
        op,
...
pub(in crate::backend::pliron) fn signed_predicate(op: InfixOp) -> ICmpPredicateAttr {
    match op {
...
pub(in crate::backend::pliron) fn unsigned_predicate(op: InfixOp) -> ICmpPredicateAttr {
    match op {
        InfixOp::Eq => ICmpPredicateAttr::EQ,
        InfixOp::Ne => ICmpPredicateAttr::NE,
...
pub(in crate::backend::pliron) fn float_predicate(op: InfixOp) -> FCmpPredicateAttr {
    match op {
...
pub(in crate::backend::pliron) fn no_overflow_flags() -> IntegerOverflowFlagsAttr {
...
pub(in crate::backend::pliron) fn record_last_uses(
...
pub(in crate::backend::pliron) enum SubscriptActual<'a> {
    Reg(Reg, Option<&'a MirPlace>),
    Descriptor(Value),
}

/// The checker-virtual slice-descriptor struct name behind `ty`, if any.
pub(in crate::backend::pliron) fn slice_struct_name(ty: &Ty) -> Option<&str> {
    match ty {
        Ty::Struct(name, _)
...
pub(in crate::backend::pliron) fn operand_regs(instr: &MirInstr) -> Vec<Reg> {
    ...
    pub(super) fn subscript_arg_regs(arg: &crate::mir::MirSubscriptArg, out: &mut Vec<Reg>) {
        match arg {
...
pub(in crate::backend::pliron) fn terminator_regs(term: &MirTerm) -> Vec<Reg> {
    match term {
...
pub(in crate::backend::pliron) fn instr_name(instr: &MirInstr) -> &'static str {
    match instr {
