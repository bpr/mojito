...
impl Checker {
    ...
    pub(super) fn materialized_reference_actual(
        &self,
        expr: &Expr,
    ) -> Result<crate::origin::RefTy, TypeError> {
        use crate::origin::{Mutability, Origin, OriginPlace, RefTy};
        match self.reference_actual(expr) {
                ...
                if message.contains("reference binding to a non-place expression") =>
            {
                ...
                let owner = match adjustments.get(&expr.source_span()) {
                    Some(crate::checked::SemanticAdjustment::MaterializeBorrowSource { owner }) => {
                        *owner
                    }
                    ...
                    None => {
                        ...
                        adjustments.insert(
                            expr.source_span(),
                            crate::checked::SemanticAdjustment::MaterializeBorrowSource { owner },
                        );
                        owner
                    }
                };
            ...
            for (field, ty) in &info.fields {
                ...
                let carries = field_carries_origin_param(ty, id)
                    || info
                        ...
                        .get(field)
                        .is_some_and(|pairs| pairs.iter().any(|(_, enclosing)| *enclosing == id.0));
                if !carries {
                    continue;
                }
    ...
    fn delegated_receiver_binder_map(&self, object: &Expr) -> Option<Vec<(u32, u32)>> {
    ...
    pub(super) fn bake_value_transfer_effects(&self, name: &str, ty: Ty) -> Ty {
    ...
    pub(super) fn record_call_through(&self, callee_name: &str, callee_ty: &Ty, args: &[Expr]) {
    ...
    pub(super) fn apply_call_through_effects(
        &self,
        callee: &str,
    ...
    fn frame_callable_identity(&self, name: &str) -> Option<crate::checked::CallThroughCallee> {
        use crate::checked::CallThroughCallee;
        let frames = self.transfer_frames.borrow();
    ...
    fn named_callable_effects(&self, name: &str) -> Vec<crate::checked::TransferEffect> {
    ...
    pub(super) fn replay_transfer_effects(
        &self,
    ...
    pub(super) fn owner_in_enclosing_scope(&self, owner: crate::origin::OwnerId) -> bool {
        ...
        match origin {
            Origin::Place(place) => {
                ...
                self.owner_in_enclosing_scope(place.root).then(|| {
                    SigOrigin::Bound(Origin::Place(crate::origin::OriginPlace {
                        root: place.root,
                        path: Vec::new(),
                    }))
                })
...
/// origin clause.
fn unqualified_struct_binder(name: &str) -> TypeError {
    TypeError::Unsupported(format!(
...
pub(super) fn origin_binder_name(expression: &Expr) -> Option<&str> {
    match &expression.kind {
...
fn sig_origin_for_binder(
    ...
    Some(match members.as_slice() {
        [] => SigOrigin::Bound(crate::origin::Origin::Param(crate::origin::OriginParamId(
            origin_param_index as u32,
        ))),
        [single] => single.clone(),
        _ => SigOrigin::union(members),
    })
...
pub(super) fn translate_call_through(
    effects: &[crate::checked::TransferEffect],
...
pub(super) fn callable_value_argument(
    decls: &[crate::types::ParamDecl],
    decl_name: &str,
    param_args: &[crate::ast::ParamArg],
) -> Option<String> {
    use crate::ast::ParamArg;
    fn argument_name(argument: &ParamArg) -> Option<String> {
        match argument {
